...
function getAIResponse(userInput) {
    ...
    MASTER_BRAIN.forEach(item => {
        ...
        item.k.forEach(keyword => {
            // Check if input tokens contain this keyword
    ...
    // Score Threshold (at least some relevance)
    if (winner && bestScore >= 5) {
        return { r: winner.r, b: winner.b || [] };
    }

    // FALLBACK: INTELLIGENT DISCOVERY
    if (raw.includes('?')) {
        return {
...
function renderBotResponse(html, buttons = []) {
    ...
    if (buttons && buttons.length > 0) {
        ...
        buttons.forEach(label => {
            ...
            b.innerText = label;
            b.style.cssText = "padding:7px 15px; border:1px solid #ea3671; border-radius:20px; background:#fff; color:#ea3671; font-size:0.85rem; cursor:pointer; font-weight:700; transition:all 0.2s;";
